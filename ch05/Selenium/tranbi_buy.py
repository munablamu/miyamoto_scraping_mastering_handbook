...
def get_project_url(driver: WebDriver) -> list:
...
def get_info(driver: webdriver) -> dict:
    result = dict()
    result['title'] = driver.find_element(By.CLASS_NAME, 'nwBuyDetail__title').text
    result['genre'] = driver.find_element(By.CLASS_NAME, 'nwBuyDetail__tagList').text
    ...
    result['sales_subjet'] = table_info[0].text
    result['fiscal_year'] = table_info[1].text

    return result


def get_detail_info(driver: WebDriver) -> None:
